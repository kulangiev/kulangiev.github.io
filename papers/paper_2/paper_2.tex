...
\label{eq:pressure}
\end{equation}
The equation of state is the ratio of this pressure to the energy density $\epsilon = \rho_{\mathrm{mass}}c^2 = \rho V'(\rho) + V(\rho)$~(Eq.~\ref{eq:rho_mass_def}), formed with the mass density rather than the Lagrangian variable $\rho_0$. In this ratio, $b$ cancels. The background condition $\rho_c = e\rho_0$---the same one that fixes $c_s = c$ and $\alpha = -1$---is the extremum condition $V'(\rho_0) = 0$, at which $P = -V(\rho_0)$ and $\epsilon = +V(\rho_0)$:
\begin{equation}
    \boxed{\;w = \frac{P}{\epsilon} = \frac{\rho_0 V'(\rho_0) - V(\rho_0)}{\rho_0 V'(\rho_0) + V(\rho_0)} = -1\;}
\label{eq:w_exact}
\end{equation}
exactly and independently of $b$. The value $w = -1$ is a consequence of the vacuum sitting at the extremum of its self-interaction potential, not of any tuning. Through the first law of thermodynamics, the expansion of cosmic volume performs work against this tension, and the cosmological density parameter scales as $\rho \propto a^{-3(1+w)} = a^0 = \text{const}$. The mechanism by which this is realized---whether through open-system condensate accretion, intrinsic potential-minimum stability, or another mechanism---is beyond the scope of the present analysis and is developed elsewhere.
...
(4) The first-order density depletion from the PG flow, $\epsilon = -2U$, decouples from the temporal metric at the background exponent $\alpha = -1$, so the vacuum around a gravitating mass is gravitationally indistinguishable from free vacuum at the $10^{-12}$ level even though its density is perturbed at $O(U)$.
