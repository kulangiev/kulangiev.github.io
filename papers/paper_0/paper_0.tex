...
Using Planck 2018 cosmological parameters~\cite{planck_2018} ($H_0 = 67.36\;\mathrm{km\,s^{-1}\,Mpc^{-1}}$) and CODATA 2018 fundamental constants, Table~\ref{tab:forward} summarizes the test of Eq. (\ref{eq:stefan}) and the inferred Hubble constant.

\begin{table}[H]
\centering
\caption{Numerical test of $\Omega_\gamma \approx \alpha^2$ using observed quantities.}
\label{tab:forward}
\begin{tabular}{lc}
\toprule
Quantity & Value \\
...
Several features argue against pure coincidence. First, the match holds with sub-percent precision at the implied $H_0 = 68.15\;\mathrm{km\,s^{-1}\,Mpc^{-1}}$, and to 2.4\% at Planck's $H_0 = 67.36$, despite the two quantities having completely independent observational origins. Second, the predicted $H_0 = 68.15\;\mathrm{km\,s^{-1}\,Mpc^{-1}}$ falls near the Planck value and well above the TRGB determination~\cite{freedman_2021}, raising the possibility that it captures the true global expansion rate more accurately than either endpoint of the $H_0$ tension.
