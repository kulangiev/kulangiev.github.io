...
\begin{abstract}
\noindent
The relativistic logarithmic Klein-Gordon equation for the vacuum condensate possesses a global $U(1)$ phase symmetry. We show that promoting this to a local gauge symmetry---the minimal coupling $\partial_\mu S \to \partial_\mu S - eA_\mu$---produces Maxwell's equations as the Euler-Lagrange equations for the gauge field $A_\mu$, with the condensate current $J^\nu = (e\meff/\heff^2)\,\rho\,u^\nu$ as source. The Madelung decomposition of the gauged equation yields the acoustic metric equations (emergent gravity) and Maxwell's equations (emergent electromagnetism) simultaneously from a single Lagrangian. Gravity is sourced by the soliton energy density (mass); electromagnetism is sourced by the topological winding number of vortex-Gausson defects, which quantizes electric charge as $Q = ne$ with $n \in \mathbb{Z}$. Both fields propagate at the sound speed $c$ on the same acoustic metric, automatically ensuring $c_{\mathrm{grav}} = c_{\mathrm{EM}} = c$ as observed. The London penetration depth of the condensate determines the electromagnetic screening length; in the electrically neutral vacuum background, it diverges, permitting free propagation of electromagnetic waves. Near vortex cores, the screening becomes significant, producing the Coulomb field as a condensate boundary effect. Gravity and electromagnetism thereby emerge from a common origin in the same condensate Lagrangian---the acoustic metric and the gauge connection---while preserving their distinct physical sources: mass versus topological charge.
\end{abstract}
...
The logarithmic nonlinearity in the Klein-Gordon equation was originally introduced in nonlinear quantum theory~\cite{bialynicki_birula_1979} and developed as a model for the superfluid vacuum~\cite{zloshchastiev_2011,zloshchastiev_2020,zloshchastiev_2023}. The acoustic-metric formalism through which long-wavelength excitations of a condensate experience an emergent Lorentzian geometry was established in the analogue-gravity programme~\cite{unruh_1981,visser_1998,volovik_2003}.
...
\begin{equation}
    \Box\psi + \mu^2\psi
    - b\,\psi\,\ln\!\left(\frac{|\psi|^2}{\rho_c}\right)
    = 0
\label{eq:log_kg}
\end{equation}
...
\label{eq:L0}
\end{equation}
where $\rho_c$ is the characteristic density of the logarithmic potential. The background density $\rho_0$ is defined by the stationarity condition $V'(\rho_0) = 0$, which gives $\rho_c = e\rho_0$.
This is invariant under $\psi \to \psi\,e^{i\alpha}$ for constant $\alpha$.
