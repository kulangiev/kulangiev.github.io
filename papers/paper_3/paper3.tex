...
\begin{abstract}
\noindent
We derive the cosmological consequences of modeling the vacuum as a relativistic logarithmic superfluid. The logarithmic equation of state yields $w = -1$ natively, identifying dark energy as the thermodynamic pressure of the condensate rather than an additional cosmological component. Combining the emergent gravitational constant $\Geff = \eta c^2/(4\pi\rho_0)$~\cite{kulangiev_2026b} with the observed relation $\Omega_\gamma \approx \alpha^2$~\cite{kulangiev_2026c}, we derive a vacuum self-consistency equation $\alpha^2 H_0^2 \rho_0 = \tfrac{2}{3}\eta\,u_{\mathrm{CMB}}$ linking the fine structure constant, the Hubble parameter, the vacuum density, and the CMB photon energy density. When the vacuum density varies spatially, all local parameters---the speed of light $c_s$, the gravitational constant $G$, and the effective cosmological constant $\Lambda$---vary in concert. We show that a modest 6.5\% vacuum underdensity inside the observed KBC void suffices to produce the locally measured $H_0 \approx 73$~km\,s$^{-1}$\,Mpc$^{-1}$ from a global value of 68.15~km\,s$^{-1}$\,Mpc$^{-1}$. We prove that the CMB acoustic peak positions are exactly invariant under these variations, because both the sound horizon and the comoving distance scale linearly with the local speed of light. The framework thus automatically satisfies the most stringent constraint in observational cosmology without tuning.
\end{abstract}
...
\label{eq:log_kg}
\end{equation}
The Madelung decomposition $\psi = \sqrt{\rho}\,e^{iS/\heff}$ yields a continuity equation and a Hamilton-Jacobi equation coupling the condensate density $\rho$ and flow velocity $\mathbf{v} = \nabla S/\meff$~\cite{kulangiev_2026a}. For the logarithmic potential $V(\rho) = -b\,\rho\,\ln(\rho/\rho_c)$, where the minus sign reflects the attractive (self-binding) interaction, the relativistic sound speed is $c_s^2 = [\ln(\rhob/\rho_c) + 2]^{-1}$, normalized so that $c_s = c$ at the background density $\rho_0$~\cite{kulangiev_2026a}.

Fundamental particles are topological defects---vortex-Gaussons---with energy $E_v$ and mass $m$~\cite{bialynicki_birula_1979}. The emergent gravitational constant, derived from the collective acoustic metric of $N = M/m$ solitons, is~\cite{kulangiev_2026b}:
\begin{equation}
    \Geff = \frac{\eta\,c^2}{4\pi\rho_0}\,,\qquad \eta \equiv \frac{E_v}{mc^2}\,,
\label{eq:G_eff}
\end{equation}
which is universal (independent of $M$, $r$, and $N$) and inversely proportional to the vacuum density. Matching to Newton's constant gives $\rho_0 = \eta c^2/(4\pi G) \approx \eta \times 10^{26}$~kg/m$^3$.
...
A natural interpretation is that the condensate is an open system: a localized phase transition accreting non-condensed vacuum from an external reservoir. The expansion of the universe corresponds to the advance of the phase boundary rather than the stretching of a closed manifold. New condensate is not created from nothing; it is converted from the disordered parent vacuum, with energy crossing the boundary. This is analogous to nucleation growth in condensed matter systems, where a crystal grows by incorporating atoms from the surrounding melt while maintaining its internal density. We note this interpretation without developing it fully here.
...
The observed relation $\Omega_\gamma \approx \alpha^2$~\cite{kulangiev_2026c} can be written as:
\begin{equation}
    H_0^2 = \frac{\Tcmb^4}{\alpha^2}\cdot\frac{8\pi^3 G k_B^4}{45\hbar^3 c^5}\,.
\label{eq:H0_from_alpha}
\end{equation}
Substituting $\Geff = \eta c^2/(4\pi\rho_0)$:
\begin{equation}
    H_0^2 = \frac{2\pi^2\,\eta\,k_B^4\,\Tcmb^4}{45\,\hbar^3\,c^3\,\alpha^2\,\rho_0}\,.
\end{equation}
Recognizing the Stefan-Boltzmann energy density $u_{\mathrm{CMB}} = \pi^2 k_B^4 \Tcmb^4/(15\hbar^3 c^3)$, this rearranges to:
\begin{equation}
    \boxed{\;\alpha^2 \cdot H_0^2(\mathbf{x}) \cdot \rho_0(\mathbf{x})
    = \frac{2}{3}\,\eta\; u_{\mathrm{CMB}}(\mathbf{x})\;}
\label{eq:triangle}
\end{equation}
For $\eta = 1$, this relation holds at every spatial point, with all quantities evaluated locally. Since $\alpha$ and $\eta$ are universal constants, the equation couples the three spatially varying fields $H_0(\mathbf{x})$, $\rho_0(\mathbf{x})$, and $u_{\mathrm{CMB}}(\mathbf{x})$: a region with lower $\rho_0$ has faster local $c_s$, lower local $u_{\mathrm{CMB}}$, and higher local $H_0$.

\subsection{Origin of the numerical factor}

The factor $2/3$ has a transparent geometric origin. Substituting $\Geff = \eta c^2/(4\pi\rho_0)$ into the Friedmann critical density $\rho_c = 3H_0^2/(8\pi G)$ gives
\begin{equation}
    \rho_c = \frac{3H_0^2}{8\pi}\cdot\frac{4\pi\rho_0}{\eta\,c^2}
           = \frac{3}{2\eta}\,\frac{H_0^2\,\rho_0}{c^2}\,.
\label{eq:rho_c_condensate}
\end{equation}
The factors of $\pi$ cancel exactly: $4\pi$ from the solid angle of the Gauss-law integration over a spherical boundary (the topology of the point source), and $8\pi$ from the Einstein field equations $G_{\mu\nu} = 8\pi G\,T_{\mu\nu}$. Their ratio leaves a strict factor of~$2$, encoding the normalization that relates the relativistic tensor coupling to the Newtonian potential. The factor of~$3$ counts the spatial dimensions participating in isotropic expansion; in the Friedmann derivation from the Einstein equations, it enters as the trace of the spatial metric. In $d$ spatial dimensions, it would be replaced by~$d$.

Multiplying both sides of Eq.~(\ref{eq:rho_c_condensate}) by $c^2$:
\begin{equation}
    \rho_c\,c^2 = \frac{3}{2\eta}\,H_0^2\,\rho_0\,.
\label{eq:kinetic}
\end{equation}
This has the structure of $(d/2) \times \text{density} \times \text{velocity}^2$, i.e.\ the kinetic energy density of an isotropically expanding fluid in three dimensions, with $H_0$ playing the role of the expansion velocity per unit distance and $\rho_0$ the mass density of the medium. In the superfluid vacuum framework, this identification is literal: the critical density of the universe is the kinetic energy density of the macroscopic vacuum condensate participating in the Hubble flow. The self-consistency equation~(\ref{eq:triangle}) then follows immediately from $u_{\mathrm{CMB}} = \alpha^2\,\rho_c\,c^2$.

\subsection{Physical interpretation}

If $\Tcmb$ is the equilibrium temperature of the condensate rather than a relic initial condition, the self-consistency equation~(\ref{eq:triangle}) is not a numerical coincidence but a thermodynamic constraint: $\rho_0$ sets $\Geff$, which sets $H_0$ via the Friedmann equation, which together with $\Tcmb$ determines $\Omega_\gamma$, and the condensate's electromagnetic coupling $\alpha$ closes the loop. The relation $\Omega_\gamma = \alpha^2$ is then a \emph{consequence} of the vacuum being in thermodynamic equilibrium.

\subsection{The coupled parameter system}
\label{sec:coupled}

In the superfluid vacuum, all local physical parameters depend on the condensate density. For a region with $\rho = \rho_0(1+\delta)$ relative to the global mean:
\begin{align}
    \frac{c_{\mathrm{local}}^2}{c_{\mathrm{global}}^2} &= \frac{1}{1 + \ln(1+\delta)}\,,
\label{eq:c_variation}\\[6pt]
    \frac{G_{\mathrm{local}}}{G_{\mathrm{global}}} &= \frac{1}{(1+\delta)\,[1 + \ln(1+\delta)]}\,,
\label{eq:G_variation}\\[6pt]
    \frac{\Lambda_{\mathrm{local}}}{\Lambda_{\mathrm{global}}} &= \frac{c_{\mathrm{local}}^4}{c_{\mathrm{global}}^4} = \frac{1}{[1 + \ln(1+\delta)]^2}\,.
\label{eq:Lambda_variation}
\end{align}
Equation~(\ref{eq:c_variation}) follows from the logarithmic equation of state $c_s^2 = [\ln(\rho/\rho_c)+2]^{-1}$. Equation~(\ref{eq:G_variation}) follows from $\Geff = \eta c_s^2/(4\pi\rho_0)$. Equation~(\ref{eq:Lambda_variation}) follows from the effective cosmological constant $\Lambda = 8\pi G\,\rho_{\mathrm{vac}}\,c^2 = 2\eta\,c_s^4$, where the $\rho_0$ dependence of $G$ and $\rho_{\mathrm{vac}}$ cancel but the $c_s$ dependence survives as the fourth power.
...
\newline
$H_{\mathrm{global}} = 68.15$~km\,s$^{-1}$\,Mpc$^{-1}$ (from the $\Omega_\gamma = \alpha^2$ prediction~\cite{kulangiev_2026c}) and requiring 
\newline
$H_{\mathrm{local}} = 73.04$~km\,s$^{-1}$\,Mpc$^{-1}$ (SH0ES~\cite{riess_2022}), we find:
\begin{equation}
    \frac{\delta\rho_0}{\rho_0} \approx -6.5\%\,.
\label{eq:delta_rho_required}
\end{equation}
This corresponds to $\delta G/G \approx +15\%$, $\delta c/c \approx +3.5\%$, and $\delta\Lambda/\Lambda \approx +15\%$. The $\Lambda$ variation dominates because $\Omega_\Lambda \gg \Omega_m$ and because $\Lambda \propto c^4$ provides a steep lever arm.

A 6.5\% vacuum underdensity is a modest thermodynamic fluctuation, well within what might be expected for a Gpc-scale region in a compressible condensate. For comparison, the observed \emph{matter} underdensity in the KBC void is 20--46\%~\cite{keenan_2013}---substantially larger.

\subsection{The global anchor}

The $\Omega_\gamma = \alpha^2$ relation~\cite{kulangiev_2026c} predicts $H_0 = 68.15 \pm 0.15$~km\,s$^{-1}$\,Mpc$^{-1}$ as the \emph{global} value---the expansion rate of the undisturbed vacuum far from any void. The Planck measurement ($67.36 \pm 0.54$) is closest to this value because the CMB averages over the entire sky, probing the global vacuum. The SH0ES measurement ($73.04 \pm 1.04$) reflects the local void environment. In this picture, the Hubble tension is not a crisis requiring new physics but an environmental effect: the two measurements \emph{should} differ, and their discrepancy is the observational signature of a vacuum thermodynamic fluctuation.
...
Under a vacuum density variation $\rho_0 \to \rho_0(1+\delta)$, the speed of light transforms as $c \to c/\sqrt{1 + \ln(1+\delta)} \equiv c \cdot f$, where $f < 1$ for an underdensity. Both integrands scale linearly with $c$:
\begin{align}
    r_s &\to r_s \cdot f\,,\\
    d_C &\to d_C \cdot f\,.
\end{align}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.89\columnwidth]{cmb_invariance_plot.pdf}
...
In the superfluid vacuum framework, $\Tcmb$ is reinterpreted as the present-day equilibrium temperature of the condensate, determined by its thermodynamic properties. The photon bath is in equilibrium with the vacuum, whose electromagnetic structure is governed by $\alpha$. The photon energy density therefore scales as $\alpha^2$ of the critical density---explaining the observed relation $\Omega_\gamma \approx \alpha^2$~\cite{kulangiev_2026c} not as a numerical coincidence but as a thermodynamic prediction.

The self-consistency equation~(\ref{eq:triangle}) makes this precise: $\Tcmb$ is not a free parameter but is determined by $\alpha$, $H_0$, and $\rho_0$ through
\begin{equation}
    \Tcmb = \left(\frac{15\,\hbar^3 c^3}{2\pi^2 k_B^4}\cdot\frac{\alpha^2 H_0^2 \rho_0}{\eta}\right)^{1/4}\,.
\label{eq:T_from_triangle}
\end{equation}
Substituting the measured values gives $\Tcmb = 2.710$~K, within 0.6\% of the FIRAS measurement. The remaining discrepancy may reflect the difference between the local and global vacuum parameters.
...
The observed vacuum energy density $\rho_\Lambda \sim 10^{-27}$~kg/m$^3$ is not $\rho_0$. The condensate density $\rho_0 \sim 10^{26}$~kg/m$^3$ is the number density of particles in the ground state, not the energy density driving expansion. In BEC physics, these are distinct quantities~\cite{volovik_2003}. The effective cosmological vacuum energy depends on the \emph{perturbation} energy $\delta\rho\,c^2$ associated with macroscopic flows, not on the background $\rho_0$~\cite{zloshchastiev_2020}.

The 120-order-of-magnitude discrepancy between Planck-scale estimates and the observed $\rho_\Lambda$ is recast in this framework: the relevant ratio is $\rho_\Lambda/\rho_0 \sim 10^{-53}$, which encodes the ratio of the cosmological expansion energy to the condensate rest energy. Whether this ratio has a natural explanation within the logarithmic model remains an open question.

\subsection{Mach's principle}

The relation $\Geff \propto 1/\rho_0$ realizes a
...
KBC void, a $6.5\%$ vacuum underdensity produces
$\delta G/G \approx +15\%$, $\delta c/c \approx
+3.5\%$, and $\delta\Lambda/\Lambda \approx +15\%$,
...
The coupled parameter system (Sec.~\ref{sec:coupled}) predicts that the speed of light inside the KBC void is $\sim 3.5\%$ faster than the cosmic mean. Photons from distant supernovae at $z \sim 0.5$--$1$ travel through the global vacuum before entering our local void, crossing a refractive boundary where $c_s$ changes. In classical analogue gravity, such a transition preserves frequency while stretching
...
\subsection{The topology of the condensate}
\label{sec:topology}

Two interpretations of the long-term expansion dynamics are available. In the open-system picture, the condensate is a bubble accreting fluid from a finite external Bulk. As the Bulk depletes, $H$ decreases monotonically and the expansion eventually ceases, with $w = -1 + \epsilon(t)$ where
$\epsilon \to 0^+$ as $t \to \infty$.

An alternative---and physically more natural---possibility is that the condensate has the topology of a persistent superfluid current: a closed, toroidal flow with quantized circulation
$\oint \mathbf{v} \cdot d\mathbf{l} = n\,h/\meff$. In this picture, the Hubble flow is the tangential velocity of the persistent current, which is topologically protected against decay. There is no boundary and no Bulk depletion; the condensate recirculates through a closed manifold. The equation
of state is $w = -1$ exactly, and $H$ asymptotes to a constant set by the circulation quantum and the torus geometry.

The toroidal topology makes specific predictions: a preferred axis (the torus symmetry axis), suppressed power at the largest CMB multipoles ($\ell = 2, 3$) from modes that do not fit within the compact dimension, and potentially detectable matched
circles in the CMB sky. The observed anomalous alignment of low-multipole CMB modes and the suppressed quadrupole power are qualitatively consistent with this picture. Distinguishing the
two topologies observationally requires precision measurements of $w(z)$: the open-system predicts slow evolution toward $-1$, while the toroidal topology predicts $w = -1$ at all epochs.

\subsection{The cosmological timeline as a phase transition}
\label{sec:timeline}

The superfluid vacuum framework suggests a unified interpretation of the cosmological timeline. At $t = 0$, a critical bubble of logarithmic condensate nucleates from the disordered Bulk. The condensate's native equation of state $w = -1$ drives exponential
expansion of the bubble interior---this is inflation, arising from the same physics as the present-day dark energy rather than from a separate scalar field.

The phase transition releases latent heat, which thermalizes into high-frequency condensate excitations (radiation). As this radiation dominates the energy budget, the effective equation of state transitions from $w = -1$ to $w = +1/3$, ending inflation and
initiating the radiation era. Solitons (particles) condense from the cooling phonon bath as temperature drops. As radiation and matter dilute through expansion, the constant condensate pressure $\rho_0
c^2$ eventually re-emerges as the dominant energy component---the current epoch of accelerated expansion.

The full sequence is:
\begin{equation}
    w = -1 \;\xrightarrow{\;\text{latent heat}\;}\;
    +\tfrac{1}{3} \;\xrightarrow{\;\text{dilution}\;}\;
    0 \;\xrightarrow{\;\text{dilution}\;}\;
    -1\,.
\end{equation}
Inflation and dark energy are the same condensate pressure, separated by the epoch during which the latent heat products dominate. The ``coincidence problem''---why $\Omega_\Lambda \approx
\Omega_m$ today---is resolved: the current epoch is simply when the latent heat has diluted back to the level of the background condensate pressure, an event that occurs exactly once in any expanding condensate.

Quantifying this picture---deriving the e-folding count from the latent heat ratio, the spectral index from the phase transition dynamics, and the reheating temperature from the energy balance---requires a detailed model of the Bulk-to-condensate
transition, which we defer to future work.

Two speculative extensions deserve mention. First, the acoustic horizon at $v = c_s$ in the Painlev\'{e}-Gullstrand flow naturally identifies black hole horizons with phase boundaries, raising the possibility of nested condensate structures. Second,
a condensate bubble nucleated with nonzero angular momentum would break statistical isotropy at the largest scales, potentially connecting to the observed anomalous alignment of low-multipole CMB modes. We explore both directions elsewhere.
...
The logarithmic equation of state gives $w = -1$ in the
topologically protected (toroidal) limit. If the condensate is instead an open system accreting from a finite Bulk, $w = -1 + \epsilon(t)$ with $\epsilon \ll 1$ approaching zero from above (Sec.~\ref{sec:topology}). In neither case does $w$ cross the phantom divide $w = -1$ from below. Upcoming surveys (DESI, Euclid, Roman) measuring $w(z)$ with percent-level precision will distinguish the two topologies: $w = -1$ at all epochs favors the toroidal picture, while slow evolution toward $-1$ favors the open system.

\paragraph{Spatial variation of $\Lambda$} Unlike standard $\Lambda$CDM, the effective cosmological constant varies spatially as $\Lambda \propto c_s^4$. A 6.5\% vacuum underdensity produces $\sim 15\%$ variation in $\Lambda$. This is testable by comparing expansion rate measurements in void vs.\ wall environments.
...
(2) The vacuum self-consistency equation $\alpha^2 H_0^2 \rho_0 = \tfrac{2}{3}\eta\,u_{\mathrm{CMB}}$ unifies the fine structure constant, the Hubble parameter, the vacuum density, and the CMB photon energy density in a single relation.

(3) When $\rho_0$ varies spatially, the speed of light, the gravitational constant, and the effective cosmological constant all vary in concert. The $\Lambda \propto c^4$ dependence provides a steep lever arm: a modest 6.5\% vacuum underdensity inside the KBC void suffices to produce the full Hubble tension.
...
(6) The cosmological timeline is reinterpreted as stages of a single phase transition: inflation and dark energy are the same condensate pressure, separated by the epoch during which the latent heat products (radiation, matter) dominate.
